...
export default function ClientGrowthIntelligencePanel({ client }: { client: Client }) {
  ...
  const WORKSPACE_TABS = [
    ...
    { id: 'website' as const, label: 'Website', Icon: Monitor },
    { id: 'seo' as const, label: 'SEO', Icon: Search },
    { id: 'gbp' as const, label: 'GBP', Icon: MapPin },
    { id: 'ads' as const, label: 'Ads', Icon: Megaphone },
    { id: 'account' as const, label: 'Account', Icon: BookOpen },
  ];

  const websiteHasWork = !!(client.websiteWorkstream?.blueprint || client.websiteWorkstream?.generatedSite);
  const seoHasWork = !!(client.seoStrategy || client.keywordStrategy);
  const gbpHasWork = !!(client.gbpStrategy);
  const adsHasWork = !!(client.adsStrategy);

  return (
    <div className="flex flex-col h-full">
      {/* ── Workspace mode navigation ── */}
      <div className="shrink-0 border-b bg-background px-3 pt-2 pb-0">
        <div className="flex items-center gap-0.5">
          {WORKSPACE_TABS.map(({ id, label, Icon }) => {
            const active = workspaceMode === id;
            const hasWork = id === 'website' ? websiteHasWork : id === 'seo' ? seoHasWork : id === 'gbp' ? gbpHasWork : id === 'ads' ? adsHasWork : false;
            return (
              <button
                key={id}
                onClick={() => setWorkspaceMode(id)}
                data-testid={`btn-workspace-${id}`}
                className={`relative flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-t-md border-b-2 transition-all ${
                  active
      ...
      {/* ── Website mode: full-height dedicated workspace ── */}
      {workspaceMode === 'website' && (
      ...
      {workspaceMode === 'overview' && (
        <ScrollArea className="h-full">
          <div className="p-4 space-y-4">
            ...
            <ClientIntelligencePanel client={client} briefRunning={briefRunning} onRefresh={refetchBrief} />

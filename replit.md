...
- **Bullpen Command Center**: Persistent thread-based command interface embedded in the Bullpen page. Supports: screenshot upload (5MB, PNG/JPEG/WebP/GIF), video upload (50MB, MP4/WebM/QuickTime), live voice dictation (Web Speech API, en-AU, text flows into composer), structured thread creation (category, route/page, priority), and Bullpen AI synthesis responses. Media stored server-side in Firebase Storage at `orgs/{orgId}/bullpen/{threadId}/{uuid}.ext` via `/api/bullpen/upload` (multer memory storage + Admin SDK). AI responses from `/api/bullpen/synthesize` use gpt-4o-mini with vision (if screenshot present), returning: diagnosis, owner, supporting roles, next action, implementation logic, risks, status, routing rationale — all written as structured Bullpen message in Firestore. Thread data at `orgs/{orgId}/bullpenThreads/{threadId}`, messages at `messages/` subcollection. Both collections use `onSnapshot` for live updates. Auth-gated: `requireOrgAccess + requireManager`.

...
- **True Server-Side Autopilot Execution Layer**: Background engine that picks up `auto_created` sweep actions, re-checks autopilot policy at execution time, acquires Firestore leases, runs low-risk handlers (cadence reminder, churn/upsell/referral flags, activity log, draft queue, approval queue), writes full attempt records with retry/backoff, and surfaces job state in a premium inspection workspace. Connected to the sweep scheduler so execution runs automatically after each sweep. Accessible at `/autopilot-execution`. Collections: `autopilotExecJobs`, `autopilotExecAttempts`, `autopilotExecLeases`.

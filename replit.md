...
- **Website ZIP Export & Launch Tab**: `GET /api/clients/:clientId/export-site.zip` — archives all HTML pages, local pages, sitemap.xml, robots.txt, decoded asset images, gallery images, and README.md using `archiver`. `POST /api/clients/:clientId/set-custom-domain` saves domain to `websiteWorkstream.customDomain`. "Launch" tab shows 7-point readiness score/checklist, custom domain input, Download ZIP button, DNS setup steps, Google Search Console setup steps, and 10-item post-launch checklist.
